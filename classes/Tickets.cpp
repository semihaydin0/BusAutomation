...
EditAGAIN:
	...
	if (TicketData.is_open())
	{
		...
		{
			int line = 0;
			cout << "Sadece koltuk bilgilerini değiştirebilirsiniz.\nMevcut koltuk numarası :" << seat_;
	...
	if (Seats.is_open())
	{
		...
		CustomerChoose:
			...
			if (layout_ == 46)
			{
				...
				cout << "Lütfen koltuk seçimi yapınız.\nSeçiminiz :";
				cin >> Choose;
				status = 0;
	...
	if (UserTickets.is_open())
	{
		...
		else
		{
			...
			if (UserExpData.is_open())
			{
				...
				cout << "Toplamda " << count << " adet kayıt bulundu." << endl;
			}
			else FileExp();
		}
	}
